...
class Subset {
  ...
  static fromJson (data: SubsetResponse) {
    let dimension: string, hierarchy: string

    if (data.Name) {
      dimension = data.UniqueName.substring(1, data.UniqueName.indexOf('].['))
      hierarchy = data.UniqueName.substring(
        data.UniqueName.indexOf('].[') + 3,
        data.UniqueName.lastIndexOf('].[')
      )
    } else {
    ...
    return new Subset(
      ...
      dimension,
      hierarchy,
      data.Elements?.map((e: ElementResponse) => e.Name) ?? [],
      data.Alias,
      data.Expression,
      data.UniqueName
    )
  ...
  private constructBodyStatic () {
    ...
    ] = `Dimensions('${this.dimensionName}')/Hierarchies('${this.hierarchyName}')`
...
interface SubsetsResponse {
  value: SubsetResponse[]
}

export { Subset, SubsetResponse, SubsetsResponse }
